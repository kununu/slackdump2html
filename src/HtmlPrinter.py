...
class HtmlPrinter:
    ...
    def print(self):
        ...
        html3 = "    </head>\n"
        html3 += "    <body>\n"
        html3 += "        <h1>#" + self.slack_data.channel_name + " chat history</h1>\n"
        ...
        if not os.path.exists("out"):
            os.makedirs("out")
        file_name = "out/" + self.slack_data.channel_name + ".html"
    ...
    def print_date_block(self, date: str) -> str:
        return '        <div class="date-block"><span class="date-block-content">' + date + '</span></div>\n'

    def print_message(self, message: SlackMessage) -> str:
        html = '        <div class="message-container">\n'
        html += self.print_user_image(message.user)
        html += '            <p class="meta">\n'
        html += '                <span class="author">' + message.user + '</span>\n'
        html += '                <span class="date">' + self.to_time(message.date) + '</span>\n'
        html += '            </p>\n'
        html += '            <p class="message">' + self.format_message(message.text) + '</p>\n'
    ...
    def print_user_image(self, user: str) -> str:
        if " " in user:
            ...
            name = user[0].upper()
        return '            <div class="user-image color' + str(self.calc_color_num(user)) + '">' + name + '</div>\n'
    ...
    def create_html_url_with_alias(self, match_obj):
        if match_obj.group(1) is not None and match_obj.group(2) is not None:
            return '<a href="' + match_obj.group(1) + '">' + match_obj.group(2) + '</a>'

    def create_html_url(self, match_obj):
        if match_obj.group(1) is not None:
            return '<a href="' + match_obj.group(1) + '">' + match_obj.group(1) + '</a>'
    ...
    def create_channel_tag_with_alias(self, match_obj):
        if match_obj.group(1) is not None:
            return '<span class="channel-mention">' + match_obj.group(1) + '</span>'

    def make_bold(self, match_obj):
        if match_obj.group(1) is not None:
            return '<b>' + match_obj.group(1) + '</b>'
    ...
    def get_custom_emoji_html(self, emoji_name: str):
        if emoji_name in self.slack_data.emojis:
            ...
            return '<i class="emoji emoji-' + emoji_name + '"></i>'
        else:
            return emoji.emojize(':' + self.data_cleaner.replace_emoji_name(emoji_name) + ':', language="alias")

    def print_replies(self, message: SlackMessage) -> str:
        html = ""
        if len(message.replies) > 0:
            html += '		    <div class="thread">\n'
            html += '               <p class="thread-meta">' + str(len(message.replies)) + ' answers </p>\n'
    ...
    def print_reply(self, reply: SlackThreadMessage) -> str:
        ...
        html += '		            <p class="meta">\n'
        html += '		                <span class="author">' + reply.user + '</span>\n'
        html += '		                <span class="date">' + self.to_datetime(reply.date) + '</span>\n'
        html += '		            </p>\n'
        html += '		            <p class="message">' + self.format_message(reply.text) + '</p>\n'
    ...
    def print_custom_emoji_definitions(self) -> str:
        ...
        for emoji_name in self.used_custom_emojis:
            html += '        .emoji-' + emoji_name + ' {\n'
            html += '          background-image: url("data:' + self.slack_data.emojis[emoji_name] + '");'
            html += '        }\n'
        html += '      </style>\n'
        return html
